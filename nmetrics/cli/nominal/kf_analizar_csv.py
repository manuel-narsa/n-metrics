...
from nmetrics.nominal.kf_core import calcular_estadisticas_kf, calcular_kf_poblacion_asintotica

def main():
    parser = argparse.ArgumentParser(description="🔍 ANÁLISIS INDIVIDUAL: Kappa de Fleiss (KF)")
    ...
    print("=" * 115)
    print(f"🚀 ANÁLISIS DEL COEFICIENTE KF (Kappa de Fleiss y Bootstrap Clásico)")
    print("=" * 115)
    ...
    kf_val = stats_kf['KF Muestra']
    ic_inf = stats_kf['IC Inf']
    ic_sup = stats_kf['IC Sup']
    ...
    print(header)
    print("-" * len(header))
    print(f"| {'KF (Bootstrap Clásico)':<30} | {format_val(kf_val):>13} | {format_val(kf_pob_real):>17} | {format_val(ic_inf):>8} | {format_val(ic_sup):>8} | {format_val(ancho_ic):>8} |")
    print("-" * len(header))

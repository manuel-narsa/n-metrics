...
from nmetrics.ordinal.ako_core import calcular_ako_poblacion_asintotica, calcular_estadisticas_ako
...
def imprimir_tabla_consola(dict_ako, es_primero):
    ...
    print(fmt_str.format(
        dict_ako['k'], dict_ako['m'], dict_ako['n'], dict_ako['Experimentos'], dict_ako['Réplicas'], dict_ako['Escenario'], dict_ako['Estimador'],
        f"{dict_ako['Cobertura Población (%)']:.1f}%", f"{dict_ako['Cobertura Muestra (%)']:.1f}%",
        fmt(dict_ako['µ(Población Real)']), fmt(dict_ako['µ(Valor Muestra)']),
        fmt(dict_ako['Media IC Inf']), fmt(dict_ako['Media IC Sup']), fmt(dict_ako['Media Ancho IC']),
        f"{dict_ako['Tiempo Medio (s)']:.1f}s"
    ))

def main():
    parser = argparse.ArgumentParser(description="🚀 STRESS TEST: Análisis de Cobertura para AKO (Ordinal)")
    ...
    parser.add_argument("-o", "--output", type=str, default="ako_cobertura.csv", help="Ruta del CSV de salida (default: ako_cobertura.csv)")
    ...
    print("=" * 190)
    print(f"🚀 STRESS TEST DE COBERTURA: AKO (BC vs Población Asintótica) - n={args.muestra}")
    print("=" * 190)
    ...
    for idx, esc in enumerate(escenarios):
        ...
        for i in range(args.experimentos):
            ...
            pob_vals_reales.append(ako_pob_real)
            
            stats_ako = calcular_estadisticas_ako(matriz_empirica, args.replicas, k_escala=args.escala)
            
            v_ako = stats_ako['AKO Muestra']
            inf_ako = stats_ako['IC Inf']
            sup_ako = stats_ako['IC Sup']
            
            if not np.isnan(v_ako) and not np.isnan(inf_ako) and not np.isnan(sup_ako):
                vals_mue.append(v_ako)
                infs.append(inf_ako)
                sups.append(sup_ako)
                anchos.append(sup_ako - inf_ako)
                cobs_mue.append(1 if (inf_ako <= v_ako <= sup_ako) else 0)
                cobs_pob.append(1 if (inf_ako <= ako_pob_real <= sup_ako) else 0)
        ...
        res_ako = {
            ...
            'Escenario': esc, 'Estimador': 'AKO (BC)',
        ...
        imprimir_tabla_consola(res_ako, es_primero=(idx == 0))
        todos_resultados.append(res_ako)

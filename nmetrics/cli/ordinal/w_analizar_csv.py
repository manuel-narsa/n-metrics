...
from nmetrics.ordinal.w_core import calcular_estadisticas_w, calcular_w_poblacion_asintotica

def main():
    parser = argparse.ArgumentParser(description="🔍 ANÁLISIS INDIVIDUAL: Kendall's W")
    ...
    print("=" * 115)
    print(f"🚀 ANÁLISIS DEL COEFICIENTE W (Kendall W y Bootstrap Clásico)")
    print("=" * 115)
    ...
    w_val = stats_w['W Muestra']
    ic_inf = stats_w['IC Inf']
    ic_sup = stats_w['IC Sup']
    ...
    print(header)
    print("-" * len(header))
    print(f"| {'Kendall W (Bootstrap Clásico)':<30} | {format_val(w_val):>13} | {format_val(w_pob_real):>17} | {format_val(ic_inf):>8} | {format_val(ic_sup):>8} | {format_val(ancho_ic):>8} |")
    print("-" * len(header))

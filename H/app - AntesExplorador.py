...
# ==============================================================================
        ...
        # Se coloca debajo de st.dataframe(df_resultados)
        # ==============================================================================
        st.markdown("---")
        st.markdown("### 📊 Explorador Termodinámico Interactivo")
        st.write("Utiliza los filtros para auditar el comportamiento de los estimadores en distintos hiperespacios.")
        
        # 1. Asegurarnos de que el Bias (Sesgo) está calculado en el DataFrame
        # Ajusta los nombres de las columnas a los que genere exactamente tu código
        if "Sesgo (Bias)" not in df_resultados.columns:
            # Asumiendo que tus columnas se llamen así, ajusta si es necesario:
            df_resultados["Sesgo (Bias)"] = df_resultados["µ (SAMPLE)"] - df_resultados["µ (POBLATION)"]
            
        # Limpiar la columna TARGET COVERAGE si viene como texto (ej. "90.00 %" -> 90.0)
        if df_resultados["TARGET COVERAGE"].dtype == object:
            df_resultados["Target Numérico"] = df_resultados["TARGET COVERAGE"].astype(str).str.replace('%', '').str.replace(',', '.').astype(float)
        else:
            df_resultados["Target Numérico"] = df_resultados["TARGET COVERAGE"]

        # 2. Controles de Interfaz (Filtros)
        col_f1, col_f2, col_f3 = st.columns(3)
        
        with col_f1:
            metrica_sel = st.selectbox(
                "Métrica a visualizar:", 
                ["Sesgo (Bias)", "POPULATION COVERAGE (%)"]
            )
            
        with col_f2:
            k_disponibles = sorted(df_resultados['k'].unique())
            k_sel = st.selectbox("Escala (k):", k_disponibles)
            
        with col_f3:
            m_disponibles = sorted(df_resultados['m'].unique())
            m_sel = st.selectbox("Jueces (m):", m_disponibles)

        # 3. Filtrar los datos según la selección del usuario
        df_filtrado = df_resultados[
            (df_resultados['k'] == k_sel) & 
            (df_resultados['m'] == m_sel)
        ]

        # 4. Generar el Gráfico Interactivo con Plotly
        if not df_filtrado.empty:
            import plotly.express as px
            
            # Configurar el título dinámico del eje Y
            ...
            fig = px.line(
                ...
                color="COEFFICIENT",
                markers=True,
                title=f"Evolución de {y_title} (k={k_sel}, m={m_sel})",
                labels={
                    "Target Numérico": "Consenso Objetivo (Target %)",
                    metrica_sel: y_title,
                    "COEFFICIENT": "Estimador"
                },
                color_discrete_map={
                    "NI (Marco N)": "#00CC96",          # Verde para Marco N
                    "Alpha Krippendorff": "#EF553B",    # Rojo para Krippendorff
                    "ICC(2,1)": "#636EFA"               # Azul para ICC
                }
            )
            
            # Mejorar la visualización del layout
            fig.update_layout(
                hovermode="x unified",
                xaxis=dict(tickmode='linear', dtick=10), # Forzar marcas cada 10%
                legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
            )
            
            # Si mostramos Cobertura, fijar el eje Y de 0 a 105 para mejor contexto
            if metrica_sel == "POPULATION COVERAGE (%)":
                fig.update_yaxes(range=[0, 105])
                # Añadir la línea de seguridad científica (95%)

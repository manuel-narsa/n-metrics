...
def ejecutar_calculo_optimizado(dict_estados, topologia, k_calc, replicas, estimadores, m_jueces, n_total, matriz_np):
    ...
    # --- C. MOTOR UNIFICADO: NO (Ordinal) ---
        ...
        try:
            ...
            azar_no, _, perc_no, _ = no_core.analizar_termodinamica_no(n_total, m_jueces, k_calc, valor_observed=pob_real)
            resultados_inf.append({
                "Métrica": "NO", "Muestra": no_muestra, "Pob. Real": pob_real, 
                "IC Inf": inf, "IC Sup": sup, "Ancho IC": sup - inf,
                "Valor Azar": azar_no, "Percentil (%)": perc_no, "Motor": "Límite Asintótico Central"
            })
        ...
        if "Intervalar" in topologia:
            if "AKI (Bootstrap C.)" in estimadores:
                try:
                    ...
                    resultados_inf.append({
                        "Métrica": "AKI", 
                        "Muestra": st_aki['AKI Muestra'], 
                        "Pob. Real": p_aki, 
                        "IC Inf": st_aki['IC Inf'], 
                        "IC Sup": st_aki['IC Sup'], 
                        "Ancho IC": st_aki['IC Sup'] - st_aki['IC Inf'], # Corregido
                        "Motor": "Bootstrap Clásico"
                    })
                except: pass
            if "ICC(2,1) (F-ANOVA)" in estimadores:
                try:
                    ...
                    resultados_inf.append({
                        "Métrica": "ICC(2,1)", 
                        "Muestra": st_icc['ICC Muestra'], 
                        "Pob. Real": p_icc, 
                        "IC Inf": st_icc['IC Inf'], 
                        "IC Sup": st_icc['IC Sup'], 
                        "Ancho IC": st_icc['IC Sup'] - st_icc['IC Inf'], # Corregido
                        "Motor": "F-ANOVA"
                    })
                except: pass    
        ...
        if "Nominal" in topologia:
            if "AKN (Bootstrap C.)" in estimadores:
                try:
                    ...
                    resultados_inf.append({
                        "Métrica": "AKN", 
                        "Muestra": st_akn['AKN Muestra'], 
                        "Pob. Real": p_akn, 
                        "IC Inf": st_akn['IC Inf'], 
                        "IC Sup": st_akn['IC Sup'], 
                        "Ancho IC": st_akn['IC Sup'] - st_akn['IC Inf'], # Corregido
                        "Motor": "Bootstrap Clásico"
                    })
                except: pass
            if "Kappa Fleiss (Bootstrap C.)" in estimadores:
                try:
                    ...
                    resultados_inf.append({
                        "Métrica": "KF", 
                        "Muestra": st_kf['KF Muestra'], 
                        "Pob. Real": p_kf, 
                        "IC Inf": st_kf['IC Inf'], 
                        "IC Sup": st_kf['IC Sup'], 
                        "Ancho IC": st_kf['IC Sup'] - st_kf['IC Inf'], # Corregido
                        "Motor": "Bootstrap Clásico"
                    })
                except: pass
        ...
        if "Ordinal" in topologia:
            if "AKO (Bootstrap C.)" in estimadores:
                try:
                    ...
                    resultados_inf.append({
                        "Métrica": "AKO", 
                        "Muestra": st_ako['AKO Muestra'], 
                        "Pob. Real": p_ako, 
                        "IC Inf": st_ako['IC Inf'], 
                        "IC Sup": st_ako['IC Sup'], 
                        "Ancho IC": st_ako['IC Sup'] - st_ako['IC Inf'], # Corregido
                        "Motor": "Bootstrap Clásico"
                    })
                except: pass
            if "Kendall W (Bootstrap C.)" in estimadores:
                try:
                    ...
                    resultados_inf.append({
                        "Métrica": "W", 
                        "Muestra": st_w['W Muestra'], 
                        "Pob. Real": p_w, 
                        "IC Inf": st_w['IC Inf'], 
                        "IC Sup": st_w['IC Sup'], 
                        "Ancho IC": st_w['IC Sup'] - st_w['IC Inf'], # Corregido
                        "Motor": "Bootstrap Clásico"
                    })
                except: pass
...
# ==============================================================================
    ...
    if k_escala is None: st.warning("⚠️ Debes definir el límite (k) en el panel lateral.")
        ...
        with st.spinner("Calculando límites del hiperespacio..."):
            if "Intervalar" in topologia: 
                azar_N, min_N = ni_core.calcular_azar_termodinamico_ni(dim_m, k_escala), 0.0 
        ...
        c1.metric("Mínimo Físico", f"{min_N:.4f}"); c2.metric("Azar Esperado", f"{azar_N:.4f}"); c3.metric("🏆 Percentil", f"{perc:.2f} %")
        ...
        if st.button("🔬 Iniciar Prueba", type="primary", disabled=(carga > 1000000) or not estimadores_cob):
            with st.spinner("Simulando..."):
                ...
                if res_cob:
                    ...
                    st.dataframe(df_cob.style.format({"Cob. Población (%)": "{:.1f}", "Cob. Muestra (%)": "{:.1f}", "µ(Población Real)": "{:.4f}", "µ(Valor Muestra)": "{:.4f}", "Ancho Medio IC": "{:.4f}"}), use_container_width=True)
...
# ==============================================================================
# PESTAÑA 6: DUELO DE PRECISIÓN N vs todos los demás
# ==============================================================================

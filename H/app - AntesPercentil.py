    ...
    if k_escala is None: 
        ...
        with st.spinner("Calculando límites del hiperespacio..."):
            if "Intervalar" in topologia: 
                ...
                # 🚀 CORRECCIÓN DEFINITIVA: Extraemos el azar y el suelo reales de tu función
        ...
        # Blindaje del slider: Evita desbordamientos de rango en Streamlit (min_N <= value <= 1.0)
        ...
        if "Intervalar" in topologia: 
            ...
            # 🚀 CORRECCIÓN DEFINITIVA: Extraemos el percentil real de tu función
        ...
        if st.button("🔬 Iniciar Prueba", type="primary", disabled=(carga > 1000000) or not estimadores_cob):
            with st.spinner("Simulando universos correlacionados..."):
                ...
                if res_cob:
                    ...
                    # 🚀 CORRECCIÓN SINK: Claves perfectamente alineadas con el diccionario del motor de simulación
                    formato_columnas = {
                        "Cobertura Población (%)": "{:.1f}", 
                        "Cobertura Muestra (%)": "{:.1f}", 
                        "µ(Población Real)": "{:.4f}", 
                        "µ(Valor Muestra)": "{:.4f}", 
                        "Media Ancho IC": "{:.4f}"
                    }
                    
                    # Filtrar el formato para aplicar únicamente a las columnas que existan en el DataFrame resultante
...
# ==============================================================================
    ...
    st.error("🛑 **AVISO CRÍTICO DE RENDIMIENTO:** Este análisis ejecuta simulaciones masivas combinatorias anidadas en disco. El resultado **no se calculará en segundos** y puede tardar de minutos a horas según los pasos declarados. Debes definir una ubicación física válida donde escribir el informe incremental.")
